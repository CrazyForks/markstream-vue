...
const temporaryRoot = mkdtempSync(join(tmpdir(), 'markstream-octane-types-'))
...
try {
  ...
  if (result.emitSkipped) {
    const diagnostics = [...ts.getPreEmitDiagnostics(program), ...result.diagnostics]
      ...
      .join('\n')
    throw new Error(`TypeScript skipped markstream-octane declaration generation.\n${diagnostics}`)
  }
  ...
  if (outputRoot !== null) {
    ...
    console.log(
      `markstream-octane declarations emitted (${sources.length} modules) to ${relative(
        packageRoot,
        outputRoot,
      )}.`,
    )
    ...
    if (checking && changed.length > 0) {
      throw new Error(`Stale markstream-octane TSRX declarations:\n${changed.join('\n')}`)
    }
    console.log(`markstream-octane TSRX declarations are current (${components.length} components).`)
  }
}
finally {
