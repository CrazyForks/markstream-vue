...
function isScopedSelector(selector: string) {
  return selector.startsWith(':where(.markstream-octane)')
    || selector.startsWith('.markstream-octane')
    || selector.startsWith('.dark .markstream-octane')
    || selector.startsWith('.dark :where(.markstream-octane)')
    || selector === 'body > div[id^="dmermaid-"]'
}

describe('markstream-octane css scoping', () => {
  ...
  it('keeps component selectors scoped to the renderer root', () => {
    ...
    expect(css).toContain(':where(.markstream-octane) .mode-btn')
    expect(css).toContain(':where(.markstream-octane).ms-tooltip')
    expect(css).toContain(':where(.markstream-octane).html-preview-frame__backdrop')
    expect(css).toContain(':where(.markstream-octane).mermaid-modal-overlay')
  })

  it('keeps keyframe names namespaced', () => {
    const { keyframes } = collectCssRules(css)

    expect(keyframes.filter(name => !name.startsWith('markstream-octane-'))).toEqual([])
    expect(new Set(keyframes).size).toBe(keyframes.length)
    expect(css).not.toContain('@keyframes spin')
    expect(css).not.toMatch(/animation(?:-name)?\s*:[^;{}]*(?<!markstream-octane-)spin(?![\w-])/)
  })

  it('keeps body portal roots in the renderer scope', () => {
    const portalRoots = [
      ['src/components/CodeBlockNode/HtmlPreviewFrame.tsrx', 'markstream-octane html-preview-frame__backdrop'],
      ['src/components/InfographicBlockNode/InfographicBlockNode.tsrx', 'markstream-octane fixed inset-0 z-50'],
      ['src/components/MermaidBlockNode/MermaidBlockNode.tsrx', 'markstream-octane mermaid-modal-overlay'],
      ['src/components/Tooltip/Tooltip.tsrx', 'markstream-octane ms-tooltip'],
      ['src/tooltip/singletonTooltip.ts', 'markstream-octane ms-tooltip'],
    ]
