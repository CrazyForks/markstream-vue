...
import type { OctanePropsWithChildren } from './types/octane'
...
export type MarkstreamCustomComponent<P = any> = ComponentBody<P> & {
  markstreamDisplay?: CustomComponentDisplayMode
}
export type CustomComponentMap = Record<string, MarkstreamCustomComponent>
export type StreamingComponent<TNode = any> = ComponentBody<NodeComponentProps<TNode>>
export type StreamingComponentMap = Record<string, StreamingComponent<any>>
export type HtmlComponent<P extends object = any> = ComponentBody<OctanePropsWithChildren<P>>
export type HtmlComponentMap = Record<string, HtmlComponent<any>>

type IsAny<T> = 0 extends (1 & T) ? true : false
type ComponentProps<T> = T extends ComponentBody<infer P>
  ? P
...
const STORE_KEY = '__MARKSTREAM_OCTANE_CUSTOM_COMPONENTS_STORE__'
const store: Store = (() => {
...
export function getCustomComponentDisplay(component: ComponentBody<any> | null | undefined): CustomComponentDisplayMode | undefined {
  return (component as MarkstreamCustomComponent | null | undefined)?.markstreamDisplay
}

export function withMarkstreamComponentDisplay<T extends ComponentBody<any>>(
  component: T,
...
export function defineStreamingComponents<const T extends Record<string, ComponentBody<any>>>(
  components: T & StreamingComponentDefinitions<T>,
) {
  return components
}

export function defineHtmlComponents<const T extends Record<string, ComponentBody<any>>>(
...
export function warnComponentMapConflicts(
  ...
  for (const tag of Object.keys(streamingComponents)) {
    ...
    warnedTags.add(tag)
    console.warn(
      `[markstream-octane] "${tag}" was provided in both streamingComponents and htmlComponents. `
